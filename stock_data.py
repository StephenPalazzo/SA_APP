...
def retrieve_basic_info(stock):
    ...
    RatiosTTM = requests.get(f'https://financialmodelingprep.com/api/v3/ratios-ttm/{stock}?apikey={api_key}').json()
    key_MetricsTTM = requests.get(f'https://financialmodelingprep.com/api/v3/key-metrics-ttm/{stock}?apikey={api_key}').json()
    # profile = requests.get(f'https://financialmodelingprep.com/api/v3/profile/{stock}?apikey={api_key}').json()
    millions = 1000000

    TTM_Info = {}
    TTM_Info['PE'] = RatiosTTM[0]['peRatioTTM']
    TTM_Info['Price-to-Sales'] = RatiosTTM[0]['priceToSalesRatioTTM']
    with open("stockTTM.json", "w") as outfile: 
        json.dump(TTM_Info, outfile)

    #Create empty dictionary and add the financials to it
    financials = {}
    dates = {}
    for item in range(5):
        dates[item] = IS[item]['calendarYear']
    for item in range(5):
        ...
        financials[dates[item]]['Debt'] = BS[item]['totalDebt'] / millions
        financials[dates[item]]['Net Debt'] = BS[item]['netDebt'] / millions
        ...
        financials[dates[item]]['Equity'] = BS[item]['totalEquity'] / millions
    ...
    fundamentals['5-Year CAGR'] = ((fundamentals[dates[0]]/fundamentals[dates[4]])**(1/5) - 1)
    fundamentals[f'{dates[0]} growth'] = (fundamentals[dates[0]] - fundamentals[dates[1]] ) / fundamentals[dates[1]]
    fundamentals[f'{dates[1]} growth'] = (fundamentals[dates[1]] - fundamentals[dates[2]] ) / fundamentals[dates[2]]
    fundamentals[f'{dates[2]} growth'] = (fundamentals[dates[2]] - fundamentals[dates[3]] ) / fundamentals[dates[3]]
    fundamentals[f'{dates[3]} growth'] = (fundamentals[dates[3]] - fundamentals[dates[4]] ) / fundamentals[dates[4]]

    #Export to Excel
    fundamentals.to_csv(f'stock_fundamentals.csv')
    fundamentals.to_json(f'stock_info.json')
    return fundamentals
